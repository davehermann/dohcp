...
const MAXIMUM_DNS_CLIENT_REQUESTS = 100000,
    MAXIMUM_EVENTS_STORED_PER_CLIENT = 1000,
...
class ClientHistory {
     ...
     * @param dhcpMessage - The message received from the client, or generated by the server in response
     * @param receivedFromClient - Is this message coming from the client? _DEFAULT: **false**_
     */
    public AddDHCPMessage(dhcpMessage: DHCPMessage, receivedFromClient = false): void {
        const newMessage = new DHCPEvent();

        if (receivedFromClient)
            newMessage.clientMessage = dhcpMessage;
        else
            newMessage.serverResponse = dhcpMessage;

        this.addDhcpEvent(dhcpMessage.clientHardwareIdentifier, newMessage);
    }
    ...
    public GetClientsInDHCPHistory(): Array<IDHCPClient> {
        ...
        for (const [clientId, dhcpEvents] of this.dhcpRequests.entries()) {
            const clientData: IDHCPClient = { clientId };
            clientList.push(clientData);

            for (let idx = dhcpEvents.length - 1; idx >= 0; idx--)
                if (!!dhcpEvents[idx].ipAddress) {
                    clientData.lastIP = dhcpEvents[idx].ipAddress;
                    clientData.hostname = dhcpEvents[idx].dnsHostname;
                    break;
                }
        }
